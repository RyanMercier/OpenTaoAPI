...
class Database:
    ...
    async def _migrate_in_place(self):
        """Hook for idempotent column adds on existing databases. SQLite
        doesn't support ``ALTER TABLE ADD COLUMN IF NOT EXISTS``, so the
        pattern is to PRAGMA the schema and skip what's already there.
        Empty for now since the data-layer schema is stable; trading
        tables moved to OpenTaoTrader."""
        return
